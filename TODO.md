...
**静态报告 Overview 失败原因空态提示**（过滤后无可见原因时在工具行旁显示轻量说明，替代仅 disabled），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。
