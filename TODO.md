...
**静态报告失败原因空态 metrics meta 字段预设 JSON 下载状态栏同样提示实际文件名**，或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。
