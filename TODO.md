...
**静态报告失败原因聚合表空态行**（过滤后表格显示「无匹配失败原因」占位行，与工具行 hint 呼应），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。
