...
**静态报告失败原因空态 metrics 事件环可复制选中行**（展开后点选事件复制单行，便于精简贴 issue），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。