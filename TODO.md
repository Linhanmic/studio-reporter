...
- [ ] 工具栏增加「全部展开 / 全部折叠」（失败过滤后便于扫读）
- [ ] 静态报告增加简单搜索（规格书 / 场景名），对齐 viewer 能力的子集
- [ ] 失败默认展开路径：打开含失败的场景与失败步骤（现仅部分 open）
- [ ] 打印样式：过滤隐藏块打印时的表现再验收
...
| 2026-09-11 | 文档四件套 + CI + 叶子步骤 + 包内单测 | 本 PR：工程文档、`ci.yml`、`leaf-row`、`static_render_test.go` |

## 下一任务（选定）

本 PR 合并后：**静态报告工具栏「全部展开 / 全部折叠」**（P1），再补失败默认展开路径。
