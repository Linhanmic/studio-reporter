...
**静态报告复制失败摘要含 path-style 深链 focus 抽检**（复制摘要 Markdown 中 open/hash 定位含 `/` 的场景），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。