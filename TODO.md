...
**静态报告失败原因空态 metrics 事件环导出可见子集 JSON**（仅含当前 kind 过滤后的 events，便于贴 issue 精简载荷），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。