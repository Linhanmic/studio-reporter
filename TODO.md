...
**Desktop 打开深链后静态报告实际定位 path-style focus（Electron/端到端冒烟）**，或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。