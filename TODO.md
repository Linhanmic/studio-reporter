...
**静态报告失败原因空态 metrics 导出/贴 issue 时附带报告目录相对路径提示**（若 meta 含 projectRoot，便于本地打开同目录产物），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。