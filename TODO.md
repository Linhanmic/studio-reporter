...
**静态报告失败原因空态 metrics 开启链接携带 kind 过滤**（`?emptyMetrics=1&emptyMetricsKind=escClear`，与开启深链一并分享），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。