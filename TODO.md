...
**静态报告失败原因空态 metrics 面板 meta+ 状态写入可分享 URL**（`emptyMetricsMeta=1`，便于同事打开即见次要字段），或 **GaugeStudio 消费 `@studio-reporter/discover`**（缺仓外权限），或 **真实 GitHub Release feed 抽检**（需发测试 tag），或证书到位并接线后启用签名 job。