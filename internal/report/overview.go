...
func writeFailReasonSection(b *bytes.Buffer, groups []FailReasonGroup) {
	...
	b.WriteString("<button type=\"button\" class=\"action-btn action-btn-tiny overview-empty-state-metrics-meta-fields-btn\" data-action=\"toggle-empty-state-metrics-meta-fields\" aria-expanded=\"false\" aria-controls=\"overview-empty-state-metrics-meta-fields\" title=\"自定义 meta 字段顺序与显隐（主/次/隐；localStorage 记忆）\">字段</button>")
