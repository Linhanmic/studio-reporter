...
2. 把链接发给同事：对方打开后自动显示空态 metrics 面板；可「复制 JSON / 贴 issue / 下载 JSON」；「贴 issue」含开启链接与过滤摘要；事件环默认折叠，可点「事件(N)」展开；展开后可用 kind chip（全部/clear/esc/…）过滤，可「复制可见 / 可见 JSON」导出当前列表，或点选某一事件行复制单行（精简贴 issue）。
