...
静态报告过滤只作用于规格书/场景等结构节点（大报告更跟手，搜索有短防抖）。Overview「失败原因聚合」按首条错误归类；「复制失败摘要」含聚合段。静态报告支持 URL hash 深链（`#scn:…` / `#spec:…` / `#fail-steps`）、工具栏「复制失败摘要」，以及 `/` 聚焦搜索、`j`/`k` 跳失败、工具栏「仅失败步骤」（打印/导出 PDF 所见即所打）、截图灯箱 `Esc` 关闭与 ←/→ 切换多图。
