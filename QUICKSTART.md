...
「历史」页支持搜索与 pass/fail/skip 过滤，以及「全选过滤结果 / 清除勾选」（长列表虚拟滚动）；勾选 1 次可打开所在文件夹 / 复制路径；勾选 1+ 次可导出（显示进度，可取消）或删除（确认，删除持有 hub 锁）；勾选恰好 2 次可对比，并可导出离线 HTML 分享卡片或复制 Markdown 摘要。  
