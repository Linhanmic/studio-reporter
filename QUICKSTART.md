...
设置可开启「启动时恢复上次标签页」；报告根目录保留最近 hub 列表，历史页与设置页可一键切换；默认可监视报告根目录 `history.json`，外部写入新报告时历史列表自动刷新（可关）。历史页「运行趋势」可对当前过滤窗口给出时长 sparkline / 失败率，并列出失败原因摘要与不稳定场景（摘要行/不稳定行均可打开最近失败并 focus，或「复制深链」；需归档内 `report.json`）；趋势窗口大小与不稳定场景上限在设置中可配，历史搜索/结论/失败原因过滤会自动记住。
