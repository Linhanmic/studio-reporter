...
1. 打开静态报告 Overview，在失败原因工具行点「开启」或「复制链接」（生成带 `?emptyMetrics=1` 的 URL；若已选 kind chip 还会带 `emptyMetricsKind=…`；复制后状态栏预览会明示 `kind=`（无 kind 时显示「当前未过滤」；「复制链接」按钮 title/aria 同步摘要）；保留当前 `#…` 过滤 hash）。
