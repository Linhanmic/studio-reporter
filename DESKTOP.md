...
历史工具栏「复制失败摘要」「复制摘要深链」与趋势面板「复制摘要」将当前过滤窗口的 `topFailReason` 聚合成 Markdown；有 hub 时附带各类原因最近一次失败的 `studio-reporter://open?run=&hub=&failSteps=1`（表格打开列 + 深链块）。工程入口：`studio-reporter digest --dir <hub> [--format json|markdown] [--write]`（`--write` 在 hub 写入 `fail-digest.md`/`fail-digest.json`；JSON 含 `openLinksLatest` / `openLinksAll`）。Desktop 导出 PDF/单文件成功后会刷新同一旁路文件（写失败不阻断导出）。
