...
状态：P0/P1 已落地（v0.5.2）；打包骨架已落地；P2「运行」+ 多项目/多会话 + 共享 discover 已落地；P3 插件门闸/本机检测/自动更新骨架已落地；代码签名证书与多平台原生报告树仍为路线图   
...
7. ~~P2「运行」封装 gauge~~；~~多项目/多会话~~；~~共享 discover 包~~；~~插件版本门闸~~；~~本机插件安装检测~~；~~自动更新骨架~~；下一步：代码签名 secrets，或原生报告树，或 GaugeStudio 消费 `@studio-reporter/discover`。
