...
| **历史** | `history.json` 列表、搜索/结果过滤、对比（可交换方向）、场景差异类型过滤（面板+导出同源）、对比分享卡片（离线 HTML / Markdown / JSON；导出后自动打开并抽检模板/标题/kinds）、导出后打开/显示、打开归档、打开所在文件夹、复制路径、原生删除（确认框 + hub 锁）、最近 hub 下拉切换 |
