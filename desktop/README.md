...
1. **运行 Gauge**：在运行栏选择 Gauge 项目目录、specs、可选 env →「运行 Gauge」。Desktop 解析 stdout 中的 `studio-reporter websocket:` 并自动连接 live viewer。
...
6. **设置**：报告根目录、自动跳转、倒计时、Gauge 可执行文件路径（持久化到 Electron `userData`）。

## 测试（无需 Electron 二进制）

```bash
cd desktop
npm test   # discover + settings/history + compare + paths + gauge-run
```
