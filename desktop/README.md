...
4. **历史**：设置报告根目录（含 `history.json`）后列出归档；可用「最近 hub」下拉切换；搜索 / pass/fail/skip 过滤；「全选过滤结果 / 清除勾选」；点击打开；勾选 1 次可打开所在文件夹 / 复制路径；勾选两次可对比（可交换方向），并可导出离线 HTML 分享卡片（深色/浅色/紧凑模板 + 自定义标题，偏好会记住）、复制 Markdown/JSON；导出后可打开卡片或显示文件夹；勾选后可删除（确认框；删除持有与 Go 对齐的 `.hub.lock`）。
5. **导出**：历史页「导出 PDF / 单文件 HTML」调用 CLI `generate --pdf|--single`；勾选 1+ 次则批量导出所选 `.uhilreport`，否则导出根目录最新。批量导出显示进度，可「取消导出」。可用「全选过滤结果 / 清除勾选」管理勾选（上限 50）。
