...
3. **报告**：收到 `ReportGenerated` 后跳转静态 `index.html`（可在设置中关闭自动跳转）；窗口未聚焦时发系统通知（可关）；打开报告目录时从 `report.json` 填充大纲。支持深链 `studio-reporter://open|connect|hub`。快捷键：`Ctrl/Cmd+1…4` 切页，`Ctrl/Cmd+Enter` 连接，`Ctrl/Cmd+Shift+H`/`F5` 刷新历史。
