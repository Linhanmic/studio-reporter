...
3. **报告**：收到 `ReportGenerated` 后跳转静态 `index.html`（可在设置中关闭自动跳转）；窗口未聚焦时发系统通知（可关）；打开报告目录时从 `report.json` 填充大纲。支持深链 `studio-reporter://open|connect|hub`。快捷键：`Ctrl/Cmd+1…4` 切页，`Ctrl/Cmd+Enter` 连接，`Ctrl/Cmd+Shift+H`/`F5` 刷新历史。设置支持 system/light/dark 主题。
4. **历史**：设置报告根目录（含 `history.json`）后列出归档；搜索 / pass/fail/skip 过滤；点击打开；勾选 1 次可打开所在文件夹 / 复制路径；勾选两次可对比，并可导出离线 HTML 分享卡片或复制 Markdown；勾选后可删除（确认框；删除持有与 Go 对齐的 `.hub.lock`）。
