...
- Screenshot galleries at suite / spec / scenario / step (hook + failure shots; click-to-enlarge lightbox with ←/→ navigation; toolbar “fail steps only” mode; shareable filter URL hash (`#…?q=&scenario=&failSteps=1`))
...
- **Overview** home page: project / host / OS / plugin / format / custom meta + counts + spec list + **fail-reason clusters** (group by primary error; counts, filter badges, and fail clusters sync to visible tree when filtered / fail-steps-only)
