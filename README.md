...
- **Overview** home page: project / host / OS / plugin / format / custom meta + counts + spec list + **fail-reason clusters** (group by primary error; per-row copy deep link / snippet + bulk 「复制全部摘要 / 复制全部深链」; empty-filter UX with clear / fail-only / undo + optional `?emptyMetrics=1` panel with 「复制 / 下载 JSON」 + 「隐藏」 for issue paste; counts, filter badges, and fail clusters sync to visible tree when filtered / fail-steps-only)
